...
if (len(sys.argv) < 2):
	print "USAGE: FULL_DATA_FILE.TXT"
	sys.exit()
...
feature_vals = [[0. for x in range(8)] for x in range(fLength)]
labeled_data = []
string_features = []
...
for line in aFile:
	...
	for x in range(4,9):
		if line[x] == "NA" or line[x] == ".":
			...
			feature_vals[line_counter][x-1] = line[x-1]
	labeled_data.append(line[6])
	line_counter += 1
